...
def build_translator(opt, report_score=True, logger=None, out_file="", log_probs_out_file=None):
    if out_file is None:
        out_file = codecs.open(opt.output, 'w+', 'utf-8')

        if opt.log_probs:
            log_probs_out_file = codecs.open(opt.output[:-4] + '_log_probs.txt', 'w+', 'utf-8')
    ...
    translator = Translator(model, fields, global_scorer=scorer,
                            out_file="", report_score=report_score,
